...
class ZK
{
  public:
    ...
    bool solve(double *x, double theta, Master &master, size_t maxRounds, bool gomory = true, bool zk = true, double tol = 1e-6, bool check = true);    // returns false if model is infeasible (may happen due to branching)
    ...
    Cut generate_gmi_cut(Master &master, size_t row, double yval, double *x, bool zk, bool check);    // generates a gmi cut
